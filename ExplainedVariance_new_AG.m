function [EV, REV, states_sleep, states_wake] = ExplainedVariance_new_AG(nmice, experiment, HighThresh, varargin)
%
...
%
% !!!!!!!!
% Pay attention to parameters: "highThresh", "PCnum_thresh", "splitSleep", whether you
% use dynamical binsize (MakeQfromS_AG.m) or constant pre-set one (MakeQfromS.m) - It should be updated later -, and whether you save the result "save_res".
% !!!!!!!!
...
% UPDATED:
% Added: 
%       - MakeQfromS_AG - use dynamical binsize based on theta-rythm
%       - Spikes_vis_AG - show spike-trains for each mouse
%       - CorrM_plot - plot the correlation matrices
%       - Calculate number of neurons per mouse used in the anaysis
%       - High and low thresholds on the firing rate
%       
...
lowThresh = .3;
% High threshold for taking out the neurons (in Hz). Include interneurons if empty
highThresh = [];
% Threshold on speed (in cm/s, epoch with lower are not considered)
speed_thresh = 4;
...
splitSleep = 20;
% Do you want to save results? Put '1' if yes
save_res = 1;
folders ={'E:\ERC_data'};
...
EV_REV_diff = cell(length(states_sleep),1);
REV_EV_diff = cell(length(states_sleep),1);
EV_REV_ratio = cell(length(states_sleep),1);
REV_EV_ratio = cell(length(states_sleep),1);
...
CorrM = cell(length(states_sleep),1);
...
%     if isfield(s{imouse}.PlaceCells,'idx') && length(s{imouse}.PlaceCells.idx) > PCnum_thresh
        ...
        % Bin the trains
        Q = MakeQfromS(selected_cells{imouse}, binsize); % Constant binsize
%         Q = MakeQfromS_AG(selected_cells{imouse}, binsize, nmice, imouse); % Dynamical binsize. Yet terribly slow. Please, use constant binsize.
    ...
    if mask(imouse)
        ...
        for isleep = 1:length(states_sleep)
            for iwake = 1:length(states_wake)
                [EV{isleep}{iwake}(imouse), REV{isleep}{iwake}(imouse), CorrM{isleep}{iwake}(imouse)] = ExplainedVariance(QPreSleep{imouse}{isleep},QWake{imouse}{iwake},QPostSleep{imouse}{isleep});
            end
        end
        ...
        for isleep = 1:length(states_sleep)
            for iwake = 1:length(states_wake)
                EV_REV_diff{isleep}{iwake}(imouse) = EV{isleep}{iwake}(imouse) - REV{isleep}{iwake}(imouse);
                REV_EV_diff{isleep}{iwake}(imouse) = REV{isleep}{iwake}(imouse) - EV{isleep}{iwake}(imouse);
                EV_REV_ratio{isleep}{iwake}(imouse) = EV{isleep}{iwake}(imouse)/REV{isleep}{iwake}(imouse);
                REV_EV_ratio{isleep}{iwake}(imouse) = REV{isleep}{iwake}(imouse)/EV{isleep}{iwake}(imouse);
            end
        end
    end
end
...
%% Plot spike clusters, correlation matrices
...
%% Plot bar with number of neurons per mouse. Under the maintenance now.
% f1 = figure('units', 'normalized', 'outerposition', [0 0 0.7 0.9]);
% bar_nn = bar(id_selected_cells, 0.6, 'FaceColor',[0 .5 .5],'EdgeColor',[0 .6 .6],'LineWidth',1.5);
...
% set(gca,'Xtick',[1:length(id_selected_cells)],'XtickLabel', [Dir.name]);
...
for iwake_state = 1:length(states_wake_toplot)
    ...
    for isleep_state = 1:length(states_sleep)
        ...
        MakeBoxPlot_DB(data_toplot, cols, [1 2], xticks, 1, 'ConnectDots', 0);
        ylabel('% explained');
        ...
        t1 = text(.4, .9, ['p = ' num2str(round(p, 3))], 'sc', 'FontSize', 16);
        makepretty
        ylim([0 40]);
    end
...
if split
    ...
    for iwake = 1:length(states_wake_toplot)
        ...
        MakeBoxPlot_DB(data_toplot, repmat(cols,1,numint), xt,  repmat({'EV', 'REV'}, 1, numint), 1, 'ConnectDots', 0);
        ylabel('% explained');
        ...
        t2 = text(.2, .9, ['p = ' num2str(round(p(1), 3))], 'sc', 'FontSize', 16);
        t2 = text(.8, .9, ['p = ' num2str(round(p(2), 3))], 'sc', 'FontSize', 16);
        ylim([0 50]);
        makepretty
...
if save_res == 1
    if ~isempty(highThresh)
        save([folders{1} '\EV_res_pyr.mat'],'nmice', 'EV', 'REV', 'EV_SWSsplit','REV_SWSsplit');
    else
        save([folders{1} '\EV_res_pyr_inter.mat'],'nmice', 'EV', 'REV', 'EV_SWSsplit','REV_SWSsplit');
        save([folders{1} '\EV_REV_diff.mat'], 'nmice', 'EV_REV_diff', 'REV_EV_diff');
        save([folders{1} '\EV_REV_ratio.mat'], 'nmice', 'EV_REV_ratio', 'REV_EV_ratio');
    end
end
